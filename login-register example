...
public OnPlayerConnect(playerid)
{
	...
	if(SQL::RowExistsEx("players", "p_name", ret_pName(playerid))) //Check if the name is registered in the database
	{
		...
		new handle = SQL::OpenTable_ReadEx("players", "p_name", ret_pName(playerid));
...
public OnPlayerDisconnect(playerid, reason)
{
	if(UserInfo[playerid][p_loggedin] == 1)
	{
		...
		new handle = SQL::OpenTable_Update("players", "p_id", UserInfo[playerid][p_id]);
		SQL::UpdateIntEntry(handle, "p_score", GetPlayerScore(playerid));
		SQL::UpdateFloatEntry(handle, "p_posx", UserInfo[playerid][p_posx]);
		SQL::UpdateFloatEntry(handle, "p_posy", UserInfo[playerid][p_posy]);
		SQL::UpdateFloatEntry(handle, "p_posz", UserInfo[playerid][p_posz]);
		SQL::Close(handle);
	}
	return 1;
}
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
		case DIALOG_REGISTER:
		{
			...
			new handle = SQL::OpenTable_Insert("players", true);
			SQL::InsertStringEntry(handle, "p_name", ret_pName(playerid));
			SQL::InsertStringEntry(handle, "p_password", UserInfo[playerid][p_password]);
			SQL::InsertIntEntry(handle, "p_score", 0);
			SQL::InsertFloatEntry(handle, "p_posx", 0.0);
			SQL::InsertFloatEntry(handle, "p_posy", 0.0);
			SQL::InsertFloatEntry(handle, "p_posz", 0.0);
			UserInfo[playerid][p_id] = SQL::Close(handle); //SQL::CloseHandle returns cache_insert_id() if the auto_increment clause was set to true in SQL::Insert
		...
		case DIALOG_LOGIN:
		{
	        ...
	        { 
				//Load player data
				new handle = SQL::OpenTable_Read("players", "p_id", UserInfo[playerid][p_id]);
